...
Properties
    ...
    load(InputStream) : 加载输入流关联的配置文件, 将配置文件中的键值对, 加载到Properties双列集合
    load(Reader)
    store(输出流, String 注释)
...
缓冲流
    ...
    字符缓冲流
    	BufferedReader(Reader) -> readLine(): 读取一行
    	BufferedWriter(Writer) -> newLine(): 写出跨平台的换行符
转换流
    InputSteamReader
    	字节流通向字符流的桥梁  =>  InputSteamReader(InputStream)
    	可以使用指定的字符集, 将字节转换成字符 => InputSteamReader(InputStream, String charsetName)
    OutputStreamWriter
    	字符流通向字节流的桥梁
    	可以使用指定的字符集, 将字符转换成字节
序列化流
...
我们在设计一个软件的时候, 会对它的结构进行划分, 大致分为两种结构, 一种叫做C/S, 一种叫做B/S, 我们来学习一下它们分别是什么.
...
![1566446300784](./总img/14/01_CS架构.png)

**B/S结构** ：全称为Browser/Server结构，是指浏览器和服务器结构。常见浏览器有谷歌、火狐等。

![1566446315067](./总img/14/02_BS架构.png)
...
计算机之间想完成通信必须遵循一种协议, 我们叫做网络通信协议, 我们来学习一下.
...
![1566446331855](./总img/14/03_通讯协议.png)
...
> **协议的介绍(TCP, UDP)**
>
> - UDP: 面向无连接, 数据不安全, 速度快(数据包大小有限制: 64K)
> - TCP: 面向有连接, 数据安全, 速度慢
...
![1566446712862](./总img/14/04_TCP协议三次握手.png)
...
![1566447182200](./总img/14/05_UDP通信过程.png)
...
![1566446503937](./总img/14/06_TCP通信过程.png)
...
Socket类构造方法
    * Socket(String host, int port) : 指定要连接服务器的IP地址和端口号
        * 根据ip地址字符串和端口号创建客户端Socket对象
        * 注意事项：只要执行该方法，就会立即连接指定的服务器程序，如果连接不成功，则会抛出异常。
            如果连接成功，则表示三次握手通过。
    
ServerSocket类:
	* ServerSocket(int port) : 指定服务器自己的端口号
...
![1566446548503](./总img/14/07_文件上传图解.png)
...
![1566446578300](./总img/14/08_BS服务器案例效果图.png)
...
![1566446643154](./总img/14/09_BS通讯图解.png)
...
那么我们今天来学习一下NIO它提供了更加高性能的数据操作方式.
