...
**通过微服务网关去测试的时候，注意：现在的令牌这里不再解析了，交给对应的微服务去解析，因为现在生成的令牌方式与之前不一样了，但是需要将token放入头中。**
...
在oauth工程的UserLoginController中添加cookie信息。
...
- 登录成功，加载用户具有权限信息（获取到用户的权限信息）
  ...
  - 实现SpringSecurity提供的接口：**UserDetailsService**接口。实现方法：loadUserByUsername(username)
...
地址：`http://localhost:8001/api/user/chuanzhi777`  
...
修改changgou-service-user的UserController的findById方法，我们先暂时把权限去掉。
...
在changgou-service-user工程的ResourceServerConfig类中，需要对请求的资源放行
...
请求地址`  http://localhost:18089/cart/add?id=1148478072756903936&num=6`
...
在changgou-user-oauth工程中添加生成带有令牌的工具类：
...
通过feign进行调用时，在请求头中携带该具体权限的令牌信息。用于调用user服务（user服务中有解析令牌的，并且可知该用户权限信息）。在changgou-user-oauth工程中添加拦截器。
...
在项目中，存在着很多的服务间的相互调用问题，那么这个时候我们可以将头信息都可以获取到并且存放到调用的Feign中，我们在FeignOauth2RequestInterceptor添加其他头信息。
