...
修改网关服务changgou-gateway-web的全局过滤器。用户如果为未登录我们可以将用户踢回到登录页面。
...
![1566656383042](./总img/11/1566656383042.png)
...
添加依赖：在changgou-service-goods工程中添加依赖 
...
在service-goods工程的SkuController中添加方法：
...
在changgou-service-goods-api工程的SkuFeign中添加方法：
...
/**
     ...
     **/
@GetMapping("/userinfo/{username}/{points}")
Result points(@PathVariable(value = "username") String username,
              @PathVariable(value = "points") Integer points);
```
