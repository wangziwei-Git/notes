...
在changgou-web-search中创建com.changgou.search.controller.SearchController,实现调用搜索，代码如下：
...
![1596209564092](./总img/7/1596209564092.png)
...
生成静态页地址 `<http://localhost:18087/page/createHtml/1148477873514881024> `
...
//绑定交换机和队列
@Bean
public Binding bindQueueToExchange(Queue goodsQueue, Exchange goodsExchange){
    return BindingBuilder.bind(goodsQueue).to(goodsExchange).with("goods-routing-key").noargs();
}
~~~

### 6.2.3 发送消息

修改SpuServiceImpl中的上架方法。
